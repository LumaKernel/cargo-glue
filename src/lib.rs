...
pub struct OptGlue {
    ...
    /// Alias for `--exclude {crates available on AtCoder 202308}`
    #[structopt(
        long,
        long_help(Box::leak(
            format!(
                "Alias for:\n--exclude {}\n ",
                ATCODER_202308_CRATES.iter().format("\n          "),
            )
            .into_boxed_str(),
        ))
    )]
    exclude_atcoder_202308_crates: bool,
...
static ATCODER_202308_CRATES: &[&str] = &[
    "https://github.com/rust-lang/crates.io-index#ac-library-rs:0.1.1",
    "https://github.com/rust-lang/crates.io-index#once_cell:1.18.0",
    "https://github.com/rust-lang/crates.io-index#static_assertions:1.1.0",
    "https://github.com/rust-lang/crates.io-index#varisat:0.2.2",
...
pub fn run(opt: Opt, ctx: Context<'_>) -> anyhow::Result<()> {
    ...
    let exclude = {
        ...
        if exclude_atcoder_202308_crates {
            exclude.extend(ATCODER_202308_CRATES.iter().map(|s| s.parse().unwrap()));
        }
    ...
    let error_message = |head: &str| {
        ...
        let crates_available_on_atcoder = iproduct!(libs_to_bundle.keys(), ATCODER_CRATES)
            .filter(|(id, s)| s.parse::<PkgSpec>().unwrap().matches_cm(&metadata[id]))
            .map(|(id, _)| format!("- `{}`\n", id))
            .join("");
        ...
        .map(
            |(lib_package, (lib_target, pseudo_extern_crate_name, mut edit))| {
                ...
                if let Some(names) = proc_macro_names.get(&lib_package.id) {
                    debug_assert!(lib_target.is_proc_macro());
                    let names = names
                        .iter()
                        .map(|name| {
                            let rename = format!(
...
pub(crate) trait MatchesCm {
    fn matches_cm(&self, pkg: &cm::Package) -> bool;
}
impl MatchesCm for PkgSpec {
    fn matches_cm(&self, pkg: &cm::Package) -> bool {
        ...
        let Some((url, src)) = self
            .url
            .as_ref()
            .zip(pkg.source.as_ref().map(|s| s.repr.as_str()))
        else {
            return true;
        };
